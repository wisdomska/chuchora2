...
/* iOS 26 (Liquid Glass) device frame — with progressive scroll-blur status bar */

// ─────────────────────────────────────────────────────────────
// Status bar — blurs progressively as content scrolls beneath it
// ─────────────────────────────────────────────────────────────
function IOSStatusBar({ dark = false, time = '9:41', scrollY = 0 }) {
  const c = dark ? '#fff' : '#000';

  // blur ramps 0→20px over first 48px of scroll; bg fades in too
  const t    = Math.min(scrollY / 48, 1);
  const blur = 6 + t * 18;                         // 6px → 24px
  const bgA  = dark
    ? `rgba(0,0,0,${(t * 0.72).toFixed(2)})`
    : `rgba(242,242,247,${(t * 0.82).toFixed(2)})`;

  // text-shadow ensures the time + icons are always legible against any content
  const shadow = dark
    ? `0 0 8px rgba(0,0,0,${0.4 + t * 0.4})`
    : `0 0 6px rgba(242,242,247,${0.5 + t * 0.4})`;
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* signal, wifi, battery */}
        <div style={{ flex: 1, height: 22, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, paddingTop: 1, paddingRight: 1, filter: `drop-shadow(${dark ? '0 0 4px rgba(0,0,0,.6)' : '0 0 4px rgba(242,242,247,.7))'})` }}>
      ...
      {/* gradient scrim — feathers content as it approaches the bar */}
      <div style={{
        position: 'absolute', top: '100%', left: 0, right: 0,
        height: 52,
        background: dark
          ? `linear-gradient(to bottom, rgba(0,0,0,${(t * 0.55).toFixed(2)}) 0%, rgba(0,0,0,0) 100%)`
          : `linear-gradient(to bottom, rgba(242,242,247,${(t * 0.65).toFixed(2)}) 0%, rgba(242,242,247,0) 100%)`,
        pointerEvents: 'none',
...
// ─────────────────────────────────────────────────────────────
// Device frame — accepts scrollY for progressive status bar blur
// ─────────────────────────────────────────────────────────────
function IOSDevice({ children, width = 402, height = 874, dark = false, title, keyboard = false, scrollY = 0 }) {
  return (
    <div style={{
      ...
      {/* status bar — always rendered on top, blur driven by scrollY */}
      <IOSStatusBar dark={dark} scrollY={scrollY} />
